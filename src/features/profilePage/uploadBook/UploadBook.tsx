...
function UploadBook() {
  ...
  function onSubmit(values: InitialValues) {
    ...
    formDataAuthor.append('Name', values.newAuthor);
    values.newAuthor && postAuthor(formDataAuthor);

    let authorsArray = values.authors.split(',');
    const genresArray = values.genres.split(',');

    if (values.newAuthor && !values.authors) {
      authorsArray = [values.newAuthor];
    }

    if (values.newAuthor && values.authors) {
      authorsArray = [...authorsArray, values.newAuthor];
    }

    const formData = new FormData();
    values.image !== null && formData.append('BookImage', values.image);
    formData.append('Title', values.title);
    (values.language === 'English' && formData.append('Language', '1')) ||
      (values.language === 'Romanian' && formData.append('Language', '2')) ||
      (values.language === 'Russian' && formData.append('Language', '3'));
    authorsArray.forEach((author) => formData.append('AuthorNames', author));
    genresArray.forEach((genre) => formData.append('GenreNames', genre));
